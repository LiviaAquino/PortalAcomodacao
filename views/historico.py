...
def historico():
    ...
    if ano == "2024":
        ...
        with st.expander("Arquivo"):
            base_2023 = st.file_uploader(
                "Anexar Base De Migração - 2023", type=["xlsx"]
            )
...
def _tabela_2024(df, df_metas, coluna, col_farol):
    ...
    df_historico["Farol"] = df_historico["Resultado"].apply(
        lambda x: "✅" if x >= 1000 else ("🟡" if x >= 800 else "❌")
    )
    ...
    df_historico = df_historico.sort_values(["Mês"])
    # filtro
    meses = ["01/2024", "02/2024", "03/2024", "04/2024", "Selecione"]
    mes = coluna.selectbox("Mês", meses, index=(len(meses) - 1))
...
def _base_mes_meta(df_atual, valor_meta):
    ...
    status["MÊS CONCLUSÃO"] = pd.to_datetime(status["MÊS CONCLUSÃO"]).dt.to_period("M")
